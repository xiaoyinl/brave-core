...
- (void)onReconcileComplete:(ledger::Result)result viewingId:(const std::string &)viewing_id type:(ledger::REWARDS_TYPE)type probi:(const std::string &)probi
{
  if (result == ledger::Result::LEDGER_OK) {
  ...
  if ((result == ledger::Result::LEDGER_OK && type == ledger::REWARDS_TYPE::AUTO_CONTRIBUTE) ||
...
- (void)saveContributionInfo:(const std::string &)probi month:(const int)month year:(const int)year date:(const uint32_t)date publisherKey:(const std::string &)publisher_key type:(const ledger::REWARDS_TYPE)type
{
