...
bool NotificationHelperMac::ShouldShowNotifications() {
  ...
  // Until we can build the browser with |mac_deployment_target| set to |10.14|
  // or above in |src/build/config/mac/mac_sdk.gni| it is not possible to detect
  // the status of notifications using the following code:
