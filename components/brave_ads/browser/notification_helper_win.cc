...
bool NotificationHelperWin::ShouldShowNotifications() {
  ...
  if (!base::FeatureList::IsEnabled(features::kNativeNotifications)) {
    LOG(WARNING) << "Native notifications are disabled in brave://flags so"
        " falling back to Message Center";
    return true;
  }

  if (!IsNotificationsEnabled()) {
    LOG(INFO) << "Notification not made: Native notifications are disabled";
    return false;
  }

  return true;
#else
  LOG(WARNING) << "Native notifications are not supported on non official"
      " builds so falling back to Message Center";
...
bool NotificationHelperWin::IsNotificationsEnabled() {
  ...
  switch (setting) {
    ...
    case ABI::Windows::UI::Notifications::
      ...
      LOG(WARNING) << "Notificatio disabled by manifest";
      return false;
    }
  }
}
