...
base::Value GetValueFromCreateParams(const CreatePlaylistParams& params) {
  ...
  create_params_value.SetKey(kPlaylistsMediaFilesKey,
...
base::Value GetValueFromPlaylistInfo(const PlaylistInfo& info) {
  ...
  playlist_value.SetStringKey(kPlaylistsMediaFilePathKey, info.media_file_path);
...
base::Value GetPlaylistValueFromPlaylistInfoJSON(
  ...
  if (playlist_info) {
    ...
    playlist.SetStringKey(
        kPlaylistsMediaFilePathKey,
        std::move(*playlist_info->FindStringKey(kPlaylistsMediaFilePathKey)));
    playlist.SetBoolKey(
...
bool PlaylistsController::DeletePlaylist(const std::string& id) {
  ...
  if (media_file_controller_->current_playlist_id() == id)
...
void PlaylistsController::OnGetOrphanedPaths(
  ...
  for (const auto& path : orphaned_paths) {
    ...
    media_file_controller_->DeletePlaylist(path);
  }
}
