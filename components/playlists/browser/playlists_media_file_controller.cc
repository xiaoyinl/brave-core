...
const base::FilePath::StringType kUnifiedMediaFileName(
...
bool AppendToFileThenDelete(const base::FilePath& source_path,
  ...
  while ((num_bytes_read =
    ...
    if (num_bytes_read !=
      ...
      VLOG(2) << __func__ << " file is exlcuded: " << source_path;
      got_error = true;
      break;
    }
  }
...
int DoGenerateSingleMediaFileOnIOThread(
  ...
  const base::FilePath unified_media_file_path =
      playlist_dir_path.Append(kUnifiedMediaFileName);

  if (base::PathExists(unified_media_file_path))
    base::DeleteFile(unified_media_file_path, false);

  base::File unified_media_file(unified_media_file_path,
  ...
  for (int i = 0; i < num_source_files; ++i) {
    ...
    if (!base::PathExists(media_file_source_path)) {
      VLOG(2) << __func__ << " file not existed: "
                          << media_file_source_path.value();
      has_skipped_source_files = true;
      continue;
    }

    if (!AppendToFileThenDelete(media_file_source_path,
                               &unified_media_file,
                               read_buffer.get(),
                               kReadBufferSize)) {
      has_skipped_source_files = true;
...
void PlaylistsMediaFileController::DeletePlaylist(
    const base::FilePath& path) {
  io_task_runner()->PostTask(FROM_HERE,
                             base::BindOnce(&DeleteDir, path));
}
...
int PlaylistsMediaFileController::GetNumberOfMediaFileSources() {
  ...
  base::Value* media_files =
      current_playlist_.FindPath(kPlaylistsCreateParamsMediaFilesPathKey);
...
void PlaylistsMediaFileController::OnMediaFileDownloaded(
  ...
  if (path.empty()) {
    // This fail is handled during the gneneration.
...
void PlaylistsMediaFileController::StartSingleMediaFileGeneration() {
  base::PostTaskAndReplyWithResult(
      ...
      FROM_HERE,
      base::BindOnce(
          &DoGenerateSingleMediaFileOnIOThread,
          playlist_dir_path_,
          media_file_source_files_count_),
      base::BindOnce(
          &PlaylistsMediaFileController::OnSingleMediaFileGenerated,
          weak_factory_.GetWeakPtr()));
}
...
void PlaylistsMediaFileController::OnSingleMediaFileGenerated(int result) {
  ...
  if (result != -1) {
    base::FilePath media_file_path =
        playlist_dir_path_.Append(kUnifiedMediaFileName);
  const std::string media_file_path_utf8 =
    ...
    current_playlist_.SetStringKey(kPlaylistsMediaFilePathKey,
                                   media_file_path_utf8);
